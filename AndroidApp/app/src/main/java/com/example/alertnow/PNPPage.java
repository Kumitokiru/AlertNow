...
public class PNPPage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.pnppage);
        ...
        if (savedUsername == null || savedMunicipality == null || savedMunicipality.isEmpty() || savedRole == null || !savedRole.equals("pnp")) {
            Log.w("PNPPage", "Invalid credentials, redirecting to LoginPage");
        ...
        try {
            ...
            Log.e("PNPPage", "Error initializing DatabaseHelper: " + e.getMessage(), e);
        ...
        barangayLabel.setText("PNP " + savedMunicipality);
        ...
        RelativeLayout rootLayout = findViewById(R.id.pnpPageLayout);
    ...
    private void initializeSocket() {
        try {
            ...
            socket.on(Socket.EVENT_CONNECT, args -> {
                Log.d("PNPPage", "Socket connected");
                try {
                    socket.emit("register_role", new JSONObject()
                            .put("role", "pnp")
                            .put("municipality", prefs.getString("municipality", "").toLowerCase()));
                } catch (org.json.JSONException e) {
                    Log.e("PNPPage", "JSON error in socket connect: " + e.getMessage());
                }
            }).on("pnp_redirect_alert", args -> {
                Log.d("PNPPage", "Received pnp_redirect_alert: " + args[0].toString());
                try {
                    JSONObject alert = (JSONObject) args[0];
                    runOnUiThread(() -> displayAlert(alert));
                } catch (Exception e) {
                    Log.e("PNPPage", "Error processing pnp_redirect_alert: " + e.getMessage());
                }
            }).on(Socket.EVENT_DISCONNECT, args -> {
                Log.d("PNPPage", "Socket disconnected, attempting to reconnect");
                socket.connect();
            }).on(Socket.EVENT_CONNECT_ERROR, args -> Log.e("PNPPage", "Connection error: " + args[0]));
            socket.connect();
        } catch (Exception e) {
            Log.e("PNPPage", "Socket initialization failed: " + e.getMessage());
        }
    }
    ...
    private void displayAlert(JSONObject alert) {
        ...
        try {
            ...
            String alertMunicipality = alert.getString("municipality");
            ...
            alertText.setText("RECEIVED ALERT\nEmergency at " + alertMunicipality);
            ...
            if (alert.has("image")) {
                ...
                alertImageView.setOnClickListener(v -> {
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(PNPPage.this);
                    ImageView dialogImage = new ImageView(PNPPage.this);
            ...
            respondButton = new Button(this);
            respondButton.setText("Respond");
            respondButton.setTextSize(20);
            respondButton.setTextColor(Color.WHITE);
            respondButton.setBackgroundColor(Color.parseColor("#3498db"));
            respondButton.setOnClickListener(v -> {
                try {
                    JSONObject responseData = new JSONObject();
                    responseData.put("alert_id", alertId);
                    responseData.put("emergency_type", type);
                    responseData.put("municipality", alertMunicipality);
                    responseData.put("lat", lat);
                    responseData.put("lon", lon);
                    String socketEvent = type.equals("Road Accident") ? "pnp_response" :
                            type.equals("Fire Incident") ? "pnp_fire_submitted" :
                                    type.equals("Crime Incident") ? "pnp_crime_response" :
                                            "pnp_response";
                    socket.emit(socketEvent, responseData);
                    statusTextView.setText("RESPONDED");
                    respondButton.setEnabled(false);
                } catch (JSONException e) {
                    Log.e("PNPPage", "JSON error in respond button: " + e.getMessage());
            ...
            Log.e("PNPPage", "JSON error in displayAlert: " + e.getMessage());
    ...
    private void playAlertSound() {
        try {
            ...
            Log.e("PNPPage", "Error playing alert sound: " + e.getMessage());
        }
    }
