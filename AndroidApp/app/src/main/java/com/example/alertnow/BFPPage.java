...
public class BFPPage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.bfppage);
        ...
        if (savedUsername == null || savedMunicipality == null || savedMunicipality.isEmpty() || savedRole == null || !savedRole.equals("bfp")) {
            Log.w("BFPPage", "Invalid credentials, redirecting to LoginPage");
        ...
        try {
            ...
            Log.e("BFPPage", "Error initializing DatabaseHelper: " + e.getMessage(), e);
        ...
        barangayLabel.setText("BFP " + savedMunicipality);
        ...
        RelativeLayout rootLayout = findViewById(R.id.bfpPageLayout);
    ...
    private void initializeSocket() {
        try {
            ...
            socket.on(Socket.EVENT_CONNECT, args -> {
                Log.d("BFPPage", "Socket connected");
                try {
                    socket.emit("register_role", new JSONObject()
                            .put("role", "bfp")
                            .put("municipality", prefs.getString("municipality", "").toLowerCase()));
                } catch (org.json.JSONException e) {
                    Log.e("BFPPage", "JSON error in socket connect: " + e.getMessage());
                }
            }).on("redirected_alert", args -> {
                Log.d("BFPPage", "Received redirected_alert: " + args[0].toString());
                try {
                    JSONObject alert = (JSONObject) args[0];
                    if (alert.getString("target_role").equals("bfp")) {
                        runOnUiThread(() -> displayAlert(alert));
                    }
                } catch (Exception e) {
                    Log.e("BFPPage", "Error processing redirected_alert: " + e.getMessage());
                }
            }).on("bfp_response", args -> {
                Log.d("BFPPage", "Received bfp_response: " + args[0].toString());
            }).on(Socket.EVENT_DISCONNECT, args -> {
                Log.d("BFPPage", "Socket disconnected, attempting to reconnect");
                socket.connect();
            }).on(Socket.EVENT_CONNECT_ERROR, args -> Log.e("BFPPage", "Connection error: " + args[0]));
            socket.connect();
        } catch (Exception e) {
            Log.e("BFPPage", "Socket initialization failed: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            if (alert.has("image")) {
                ...
                alertImageView.setOnClickListener(v -> {
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(BFPPage.this);
                    ImageView dialogImage = new ImageView(BFPPage.this);
            ...
            respondButton.setOnClickListener(v -> {
                ...
                try {
                    ...
                    Log.e("BFPPage", "Error submitting response: " + e.getMessage());
                }
            });
            ...
            Log.e("BFPPage", "Error displaying alert: " + e.getMessage());
        }
    }
