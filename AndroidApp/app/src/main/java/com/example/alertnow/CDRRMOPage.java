...
public class CDRRMOPage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.cdrrmopage);
        ...
        if (savedUsername == null || savedMunicipality == null || savedMunicipality.isEmpty() || savedRole == null || !savedRole.equals("cdrrmo")) {
            Log.w("CDRRMOPage", "Invalid credentials, redirecting to LoginPage");
        ...
        try {
            ...
            Log.e("CDRRMOPage", "Error initializing DatabaseHelper: " + e.getMessage(), e);
        ...
        barangayLabel.setText("CDRRMO " + savedMunicipality);
        ...
        RelativeLayout rootLayout = findViewById(R.id.cdrrmoPageLayout);
    ...
    private void initializeSocket() {
        try {
            ...
            socket.on(Socket.EVENT_CONNECT, args -> {
                Log.d("CDRRMOPage", "Socket connected");
                try {
                    socket.emit("register_role", new JSONObject()
                            .put("role", "cdrrmo")
                            .put("municipality", prefs.getString("municipality", "").toLowerCase()));
                } catch (org.json.JSONException e) {
                    Log.e("CDRRMOPage", "JSON error in socket connect: " + e.getMessage());
                }
            }).on("redirected_alert", args -> {
                Log.d("CDRRMOPage", "Received redirected_alert: " + args[0].toString());
                try {
                    JSONObject alert = (JSONObject) args[0];
                    if (alert.getString("target_role").equals("cdrrmo")) {
                        runOnUiThread(() -> displayAlert(alert));
                    }
                } catch (Exception e) {
                    Log.e("CDRRMOPage", "Error processing redirected_alert: " + e.getMessage());
                }
            }).on("cdrrmo_response", args -> {
                Log.d("CDRRMOPage", "Received cdrrmo_response: " + args[0].toString());
            }).on(Socket.EVENT_DISCONNECT, args -> {
                Log.d("CDRRMOPage", "Socket disconnected, attempting to reconnect");
                socket.connect();
            }).on(Socket.EVENT_CONNECT_ERROR, args -> Log.e("CDRRMOPage", "Connection error: " + args[0]));
            socket.connect();
        } catch (Exception e) {
            Log.e("CDRRMOPage", "Socket initialization failed: " + e.getMessage());
        }
    }
    ...
    private void displayAlert(JSONObject alert) {
        ...
        try {
            ...
            String type = alert.getString("emergency_type");
            ...
            if (alert.has("image")) {
                ...
                alertImageView.setOnClickListener(v -> {
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(CDRRMOPage.this);
                    ImageView dialogImage = new ImageView(CDRRMOPage.this);
            ...
            respondButton.setBackgroundResource(R.drawable.button_background);
            respondButton.setTextColor(Color.BLACK);
            respondButton.setOnClickListener(v -> {
                statusTextView.setText("RESPONDED");
                JSONObject responseData = new JSONObject();
                try {
                    responseData.put("alert_id", alertId);
                    responseData.put("role", "cdrrmo");
                    responseData.put("lat", lat);
                    responseData.put("lon", lon);
                    responseData.put("barangay", alertBarangay);
                    responseData.put("emergency_type", type);
                    responseData.put("municipality", prefs.getString("municipality", "").toLowerCase());
                    socket.emit("submit_response", responseData);
                } catch (JSONException e) {
                    Log.e("CDRRMOPage", "Error submitting response: " + e.getMessage());
                }
            });
            ...
            Log.e("CDRRMOPage", "Error displaying alert: " + e.getMessage());
        }
    }
